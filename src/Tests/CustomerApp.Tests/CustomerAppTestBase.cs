...
using CustomerApp.Migrations.SeedData;
...
namespace CustomerApp.Tests
{
    public abstract class CustomerAppTestBase : AbpIntegratedTestBase<CustomerAppTestModule>
    {
        ...
        protected void UsingDbContext(Action<CustomerAppDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<CustomerAppDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<CustomerAppDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<CustomerAppDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<CustomerAppDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CustomerAppDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<CustomerAppDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CustomerAppDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<CustomerAppDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<CustomerAppDbContext, Task<T>> func)
        {
            T result;
